...
class GenerateSelfContainedSpawnDataStage(JsonProcessingStage):
    ...
    def process(self, base_path: Path, modid: Optional[str], modtype: Optional[ModType], data: Dict[str, List[Any]]):
        ...
        halfbaked, global_random_swaps = self._expand_spawning_groups_data(data['spawn_groups'])
        ...
        for name, _ in map_names:
            ...
            # Filter containers to only operate on those used on map. This'll be a copy to avoid messing up shared source data.
            local_containers = deepcopy(list(filter(lambda x: x['bp'] in required_groups, halfbaked)))
            ...
            if random_map_swaps:
                ...
                self._evaluate_random_swaps_on_containers(local_containers, random_map_swaps_lookup)

            # Apply global random replacements if enabled on map.
            ignores_global_random_swaps = mapdata[name]['world_settings']['worldSettings'].get('onlyEventGlobalSwaps', False)
            if not ignores_global_random_swaps:
                self._apply_global_random_swaps_on_data(local_containers, global_random_swaps)

            # Apply merger groups on the filtered containers.
            self._apply_merger_groups(local_containers)
            ...
            for zone in mapdata[name]['npc_spawns']['spawns']:
                ...
                npc_count = math.ceil(npc_count * container_lookup[container_bp]['maxNPCNumberMultiplier'])
    ...
    def _expand_spawning_groups_data(self, data: List[Any]) -> Tuple[List[Any], List[Any]]:
        ...
        # Build global random replacements to be applied later.
        ...
        return (containers, random_swaps_lookup)

    def _apply_global_random_swaps_on_data(self, containers: List[Any], random_swaps_lookup: Optional[List[Any]]):
        # Evaluate global random replacements. This requires an "opt-in" depending on map's configuration (Genesis 1 disables
        # global replacements).
        if random_swaps_lookup:
            self._evaluate_random_swaps_on_containers(containers, random_swaps_lookup)
        return containers

    def _apply_merger_groups(self, containers: List[Any]):
