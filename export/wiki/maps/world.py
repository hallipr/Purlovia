...
from ark.overrides import OverrideSettings, get_overrides_for_map
...
class World(PersistentLevel):
    ...
    def ingest_level(self, level: UAsset):
        ...
        level_overrides = get_overrides_for_map(assetname, None)
        ...
        for export in level.exports:
            # Raw harvestable resources aren't exported as JSON. If this is a foliage actor, branch off to a dedicated unit.
            if str(export.klass.value.name) == 'HierarchicalInstancedStaticMeshComponent':
                try:
                    collect_harvestable_resources(export, level_overrides, self.resource_nodes)
                except Exception:
